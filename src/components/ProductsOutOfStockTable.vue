<template>
  <div class="flex flex-col">
    <div class="mb-2">
      <h2 class="font-bold text-left">List of Products out of stock</h2>
    </div>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="product in products" v-bind:key="product.id">
                ...
                >
                  <button
                    type="button"
                    @click="changeStatus(product.id, product.state)"
                    :class="[
                      product.state
                  ...
                  >
                    {{ product.state ? "Deactivate" : "Activate" }}
                  </button>
                  <router-link
...
export default {
  ...
  created: function () {
    axios
      .get("http://localhost/newco/api/products/out-of-stock.php")
      .then((res) => {
  ...
  methods: {
    deleteProduct(id) {
      if (confirm("Do you really want to delete?")) {
        axios
          .delete("http://localhost/newco/api/products/delete.php", {
            params: {
              id: id,
            },
          })
    ...
    changeStatus(id, state) {
      axios
        .put("http://localhost/newco/api/products/update-state.php", {
          id: id,
